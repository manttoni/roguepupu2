...

{
	 ...
	 * */
	bool blocks_movement(const entt::registry& registry, const Domain::Position& position)
	{
		if (ECS::get_cell(registry, position).get_type() == Domain::Cell::Type::Rock)
			return true;
	 ...
	 * */
	bool can_move(const entt::registry& registry, const Domain::Position& from_pos, const Domain::Position& to_pos)
	{
		...
		const auto corner1 = Domain::Position(from.y * cave_size + to.x, cave.get_idx());
		const auto corner2 = Domain::Position(to.y * cave_size + from.x, cave.get_idx());
	 ...
	 * */
	std::vector<Domain::Position> find_path(const entt::registry& registry, const Domain::Position& start, const Domain::Position& end, const bool allow_blocked_end)
	{
		...
		std::map<Domain::Position, Domain::Position> came_from;
		...
		{
			Domain::Position current_pos = open_set[0];
			for (const Domain::Position cell_pos : open_set)
			...
			{	// found optimal path from start to end
				std::vector<Domain::Position> path;
	...
	void move(entt::registry& registry, const entt::entity entity, const Domain::Position& position)
	{
		const auto previous = registry.all_of<Domain::Position>(entity) ?
			registry.get<Domain::Position>(entity) :
			Domain::Position::invalid();
		registry.emplace_or_replace<Domain::Position>(entity, position);
