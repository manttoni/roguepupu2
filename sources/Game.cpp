...
Game::Game() :
	level(1),
	player(EntityFactory::instance().create_entity(world.get_registry(), EntityType::player, get_cave().get_source()))
{}
...
void Game::start()
{
	...
	while (key != KEY_ESCAPE)
	{
		...
		{
			if (MovementSystem::move(get_registry(), get_player(), key) > 0)
				change_level();
		}
